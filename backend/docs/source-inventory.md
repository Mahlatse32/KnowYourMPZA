...
|---|---|---|---|
| IEC election results | `scripts/discover_iec_sources.py`; `scripts/audit_iec_structured_formats.py`; `scripts/ingest_iec_metadata_manifest.py`; `scripts/ingest_iec_vote_totals.py` (design: `docs/iec-election-results-ingestion.md`) | Discovery, structured-format audit, metadata-manifest, and vote-total reports under `reports/` | **Metadata + source manifests + local reviewed CSV vote totals** (`iec_elections`, `iec_source_manifests`, `iec_vote_totals`); CSV is the preferred parser profile after offline audit - **no live vote-total workflow, winners, office-bearers, councillors, or internal geography/party/candidate mappings** |
