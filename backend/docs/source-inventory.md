...
| Source | URL | Domain | Data type | Current status | Ingestion script | Evidence/source URL retained? | Update frequency | Risk/notes | Priority |
|---|---|---|---|---|---|---|---|---|---|
| IEC election results | https://results.elections.org.za/home/ | Elections | Election metadata, source manifests, and one audited party vote-total CSV profile | Implemented, limited | `scripts/ingest_iec_metadata_manifest.py`; `scripts/ingest_iec_vote_totals.py` | Yes: official source URL, manifest/checksum, raw source row | Manual reviewed files only | No live result workflow; no winners, office-bearers, or internal party/candidate/geography mappings | P0 |
...
|---|---|---|---|
| Current MP/member baseline sources | `scripts/audit_mp_member_sources.py` | `reports/mp_member_source_audit.json` / `.md` | Audit only - Parliament official National Assembly and NCOP listings are baseline candidates; People's Assembly is enrichment, PMG is activity support, and no expected MP universe is claimed yet |
| Official Parliament member-list profile | `scripts/profile_parliament_member_sources.py` | `reports/parliament_member_source_profile.json` / `.md` | Profile only - accepts only `parliament.gov.za` baseline candidates, records explicit field signals and parser readiness, and performs no ingestion |
| IEC election results | `scripts/discover_iec_sources.py`; `scripts/audit_iec_structured_formats.py`; `scripts/audit_iec_live_downloads.py`; `scripts/ingest_iec_metadata_manifest.py`; `scripts/ingest_iec_vote_totals.py` (design: `docs/iec-election-results-ingestion.md`) | Discovery, structured-format audit, bounded live-download audit, metadata-manifest, and vote-total reports under `reports/` | **Metadata + source manifests + local reviewed CSV vote totals** (`iec_elections`, `iec_source_manifests`, `iec_vote_totals`) plus a bounded live-download audit; CSV is the preferred parser profile after offline audit - **no live/scheduled vote-total workflow, winners, office-bearers, councillors, or internal geography/party/candidate mappings** |
