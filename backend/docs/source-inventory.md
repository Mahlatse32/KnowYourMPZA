...
|---|---|---|---|
| IEC election results | `scripts/discover_iec_sources.py`; `scripts/ingest_iec_metadata_manifest.py`; `scripts/ingest_iec_vote_totals.py` (design: `docs/iec-election-results-ingestion.md`) | Source-discovery, metadata, and vote-total reports under `reports/` | **Metadata + source manifests + local audited CSV vote totals** (`iec_elections`, `iec_source_manifests`, `iec_vote_totals`) - **no live workflow, winners, office-bearers, or internal geography/party/candidate mappings** |
